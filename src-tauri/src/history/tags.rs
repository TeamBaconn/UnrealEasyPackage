...
/// Generate a build's tag list: platform, config, target (if any), status.
pub fn generate(platform: &str, config: &str, target: &str, status: &str) -> Vec<String> {
    let mut tags = vec![platform.to_string(), config.to_string()];
...
mod tests {
    ...
    fn generate_then_reverse_round_trips() {
        let tags = generate("Win64", "Development", "SampleProjectSteam", "Success");
        assert_eq!(tags, vec!["Win64", "Development", "SampleProjectSteam", "Success"]);
        let r = reverse(&tags);
        assert_eq!(r.platform.as_deref(), Some("Win64"));
        assert_eq!(r.config.as_deref(), Some("Development"));
    ...
    fn target_is_the_leftover_even_in_any_order() {
        ...
        assert_eq!(r.config.as_deref(), Some("Shipping"));
    ...
    fn missing_target_reverses_to_none() {
        let tags = generate("Win64", "Shipping", "", "Cancelled");
